...
def decrypt(ct, key, iv):
    ...
    return pkcs7.unpad(bytes(pt_padded))
...
if __name__ == '__main__':
    ...
    assert len(encrypt(b'', key, iv)) == 16
    assert len(encrypt(b'a'*15, key, iv)) == 16
    assert len(encrypt(b'a'*16, key, iv)) == 32
    assert decrypt(encrypt(b'', key, iv), key, iv) == b''
    assert decrypt(encrypt(b'a'*15, key, iv), key, iv) == b'a'*15
    assert decrypt(encrypt(b'a'*16, key, iv), key, iv) == b'a'*16
